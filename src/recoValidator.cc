#include "Analysis/displacedMuons-Validator/src/recoValidator.h"

////////
//////// -- Constructor
////////
recoValidator::recoValidator(const edm::ParameterSet& iConfig)
{
   ...
   h_counts = new TH1F("h_counts", "", 1, 0, 2);
   ...
   // muon kinematics
   h_mu_pt = new TH1F("h_mu_pt", ";muon->pt();", 40, 0, 400);
   h_dtr_pt = new TH1F("h_dtr_pt", ";muon->pt();", 40, 0, 400);
   h_dgl_pt = new TH1F("h_dgl_pt", ";muon->pt();", 40, 0, 400);
   h_dsa_pt = new TH1F("h_dsa_pt", ";muon->pt();", 40, 0, 400);
   ...
   // tracks
   h_best_dxy_dz = new TH2F("h_best_dxy_dz", ";track()->dxy();track()->dz();", 100, -1.0, 1.0, 100, -10.0, 10.0);
   h_best_dxy = new TH1F("h_best_dxy", ";track()->dxy();", 100, -1.0, 1.0);
   ...
   h_best_dz = new TH1F("h_best_dz", ";track()->dz();", 100, -10.0, 10.0);
   ...
   e_mu_pt = new TEfficiency("e_mu_pt", ";Generated p_{T} (GeV); Efficiency", 40, 0, 400);
   e_dtr_pt = new TEfficiency("e_dtr_pt", ";Generated p_{T} (GeV); Efficiency", 40, 0, 400);
   e_dgl_pt = new TEfficiency("e_dgl_pt", ";Generated p_{T} (GeV); Efficiency", 40, 0, 400);
   e_dsa_pt = new TEfficiency("e_dsa_pt", ";Generated p_{T} (GeV); Efficiency", 40, 0, 400);
   e_or_pt = new TEfficiency("e_or_pt", ";Generated p_{T} (GeV); Efficiency", 40, 0, 400);
   e_mu_eta = new TEfficiency("e_mu_eta", ";Generated #eta ; Efficiency", nBinsEta, etaBins);
   e_dtr_eta = new TEfficiency("e_dtr_eta", ";Generated #eta ; Efficiency", nBinsEta, etaBins);
   e_dgl_eta = new TEfficiency("e_dgl_eta", ";Generated #eta ; Efficiency", nBinsEta, etaBins);
   e_dsa_eta = new TEfficiency("e_dsa_eta", ";Generated #eta ; Efficiency", nBinsEta, etaBins);
   e_or_eta = new TEfficiency("e_or_eta", ";Generated #eta ; Efficiency", nBinsEta, etaBins);
   e_mu_dxy = new TEfficiency("e_mu_dxy", ";Generated d_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dtr_dxy = new TEfficiency("e_dtr_dxy", ";Generated d_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dgl_dxy = new TEfficiency("e_dgl_dxy", ";Generated d_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dsa_dxy = new TEfficiency("e_dsa_dxy", ";Generated d_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_or_dxy = new TEfficiency("e_or_dxy", ";Generated d_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_mu_lxy = new TEfficiency("e_mu_lxy", ";Generated L_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dtr_lxy = new TEfficiency("e_dtr_lxy", ";Generated L_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dgl_lxy = new TEfficiency("e_dgl_lxy", ";Generated L_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_dsa_lxy = new TEfficiency("e_dsa_lxy", ";Generated L_{xy} (cm); Efficiency", nBinsDxy, dxyBins);
   e_or_lxy = new TEfficiency("e_or_lxy", ";Generated L_{xy} (cm); Efficiency", nBinsDxy, dxyBins);


   DisplacedToken_ = consumes<edm::View<reco::Muon> >  (parameters.getParameter<edm::InputTag>("DisplacedCollection"));
   GenToken_  = consumes<edm::View<reco::GenParticle> >( edm::InputTag("genParticles") );
...
//////// -- fillDescriptions
////////
void recoValidator::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
//////// -- Analyze
////////
void recoValidator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{

   //iSetup.get<GlobalTrackingGeometryRecord>().get(globalGeometry);
   //iSetup.get<IdealMagneticFieldRecord>().get(theField);
   //iSetup.get<TrackingComponentsRecord>().get("SteppingHelixPropagatorOpposite", propagator);
   ...
   if (iEvent.id().event() == 291) {
     for (const auto& mu : *DisplacedCollection_){
       std::cout << mu.pt() << "\t" << mu.eta() << "\t" << mu.bestTrack()->dxy() << "\t" << mu.bestTrack()->dz() << std::endl;
     }
   }
   for (const auto& mu : *DisplacedCollection_)
   {
     ...
     h_best_dxy->Fill(mu.bestTrack()->dxy());
     h_best_dz->Fill(mu.bestTrack()->dz());
     h_best_dxy_dz->Fill(mu.bestTrack()->dxy(), mu.bestTrack()->dz());

     /*
     if (mu.bestTrack()->dxy() > 0.5 && mu.bestTrack()->dxy() < 0.6 && mu.bestTrack()->dz() > 2. && mu.bestTrack()->dz() < 3.5){
   ...
   for (const auto& gp : *GenCollection_)
   {
     if (fabs(gp.pdgId()) != 13)
       continue;
     //std::cout << gp.isPromptFinalState() << std::endl;
     //if (fabs(gp.mother()->pdgId()) != 1000006)
     //  continue;
     if (!gp.isPromptFinalState())
       continue;

     //TVector3 extgp = propagateMomentum(gp, bs);
     float delR = 999.;
     reco::Muon bestMu;
     for (const auto& mu : *DisplacedCollection_){
     ...
     double _gendxy = -(gp.vx() - bs.x0())*sin(gp.phi()) + (gp.vy() - bs.y0())*cos(gp.phi());
     double gendxy = fabs(_gendxy);
     double genlxy = sqrt( (gp.vx() - bs.x0())*(gp.vx() - bs.x0()) + (gp.vy() - bs.y0())*(gp.vy() - bs.y0()) );


     if (delR < 0.2) {
       e_mu_pt->Fill(true, gp.pt());
       e_mu_eta->Fill(true, gp.eta());
       e_mu_dxy->Fill(true, gendxy);
       e_mu_lxy->Fill(true, genlxy);
     } else {
       e_mu_pt->Fill(false, gp.pt());
       e_mu_eta->Fill(false, gp.eta());
       e_mu_dxy->Fill(false, gendxy);
       e_mu_lxy->Fill(false, genlxy);
     }
     if (delR < 0.2 && bestMu.isTrackerMuon()) {
       e_dtr_pt->Fill(true, gp.pt());
       e_dtr_eta->Fill(true, gp.eta());
       e_dtr_dxy->Fill(true, gendxy);
       e_dtr_lxy->Fill(true, genlxy);
     } else {
       e_dtr_pt->Fill(false, gp.pt());
       e_dtr_eta->Fill(false, gp.eta());
       e_dtr_dxy->Fill(false, gendxy);
       e_dtr_lxy->Fill(false, genlxy);
     }
     if (delR < 0.2 && bestMu.isGlobalMuon()) {
       e_dgl_pt->Fill(true, gp.pt());
       e_dgl_eta->Fill(true, gp.eta());
       e_dgl_dxy->Fill(true, gendxy);
       e_dgl_lxy->Fill(true, genlxy);
     } else {
       e_dgl_pt->Fill(false, gp.pt());
       e_dgl_eta->Fill(false, gp.eta());
       e_dgl_dxy->Fill(false, gendxy);
       e_dgl_lxy->Fill(false, genlxy);
     }
     if (delR < 0.2 && bestMu.isStandAloneMuon() && !bestMu.isGlobalMuon() && !bestMu.isTrackerMuon() ) {
       e_dsa_pt->Fill(true, gp.pt());
       e_dsa_eta->Fill(true, gp.eta());
       e_dsa_dxy->Fill(true, gendxy);
       e_dsa_lxy->Fill(true, genlxy);
     } else {
       e_dsa_pt->Fill(false, gp.pt());
       e_dsa_eta->Fill(false, gp.eta());
       e_dsa_dxy->Fill(false, gendxy);
       e_dsa_lxy->Fill(false, genlxy);
     }
     if (delR < 0.2 && (bestMu.isStandAloneMuon() || bestMu.isGlobalMuon()) ) {
       e_or_pt->Fill(true, gp.pt());
       e_or_eta->Fill(true, gp.eta());
       e_or_dxy->Fill(true, gendxy);
       e_or_lxy->Fill(true, genlxy);
     } else {
       e_or_pt->Fill(false, gp.pt());
       e_or_eta->Fill(false, gp.eta());
       e_or_dxy->Fill(false, gendxy);
       e_or_lxy->Fill(false, genlxy);
     }
   }
...
/*
TVector3 recoValidator::propagateMomentum(reco::GenParticle gen, reco::BeamSpot bs){
  int charge = gen.charge();
  GlobalPoint vertex(gen.vx(), gen.vy(), gen.vz());
  GlobalVector momentum(gen.px(), gen.py(), gen.pz());
  GlobalTrajectoryParameters params(vertex, momentum, charge, &*theField);
  FreeTrajectoryState fts = FreeTrajectoryState(params);
  std::pair<FreeTrajectoryState, double> pairPropag = propagator->propagateWithPath(fts, bs);
  ...
  //double E = sqrt(px*px+py*py+pz*pz + gen.mass()*gen.mass());
    TVector3 genatBS(px,py,pz);
      return genatBS;
}
*/
